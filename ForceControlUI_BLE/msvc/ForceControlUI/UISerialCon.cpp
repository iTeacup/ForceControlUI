...
UISerialCon::UISerialCon(UIMainWindowBase *main_win, const char *title) : UIBaseWindow(main_win, title)
{
...
void UISerialCon::UpdateSerialLogs()
{
    std::string line = m_serial_reader.GetLog();
    while (!line.empty())
    {
        AddLogLine(line);
        line = m_serial_reader.GetLog(); // 获取下一行数据
    }
}

void UISerialCon::AddLogLine(const std::string &line)
{
    std::string log_line;
...
void UISerialCon::FlushPendingToDisplay()
{
    // 1) 合并 pending
...
void UISerialCon::Draw()
{
    ...
    char buf[64] = {0};
    ST_SerialCfg *serial_cfg = &(g_cfg->m_serial_cfg);
    if (ImGui::CollapsingHeader(u8"参数设置", ImGuiTreeNodeFlags_DefaultOpen))
    {
        ...
        ImGui::Text(buf);
        const char *combo_preview_value = serial_cfg->com_index >= 0 ? m_vec_coms[serial_cfg->com_index].c_str() : ""; // Pass in the preview value visible before opening the combo (it could be anything)
        if (ImGui::BeginCombo(u8"串口号", combo_preview_value, 0))
        {
            for (size_t n = 0; n < m_vec_coms.size(); n++)
            {
                const bool is_selected = (serial_cfg->com_index == n);
                if (ImGui::Selectable(m_vec_coms[n].c_str(), is_selected))
                {
                    serial_cfg->com_index = n;
                }
        ...
        if (ImGui::Combo(u8"波特率", (int *)&serial_cfg->baud_rate, u8" 9600\0 14400\0 19200\0 38400\0 56000\0 57600\0 115200\0"
    ...
    else
    {
        ...
        {
            if (serial_cfg->com_index < 0 || serial_cfg->com_index >= m_vec_coms.size())
            {
            ...
            {
                if (!m_serial_reader.OpenSerialPort(m_vec_coms[serial_cfg->com_index], serial_cfg->baud_rate))
                {
