...
{	
	...
	try
	{
		...
		if (inet_pton(AF_INET, mIpRemote.data(), &mRemoteAddr.sin_addr) <= 0)//远程IP
		{
...
bool CSRICommTCPClient::ConnectTCP()
{
	...
	// 控制socket的模式，FIONBIO：设置阻塞模式，非阻塞模式（blockFlag = 1）
...
#ifdef  IS_WINDOWS_OS
	if (setsockopt(mSocket, SOL_SOCKET, SO_KEEPALIVE, (const char *)&keepAlive, sizeof(keepAlive)) < 0)//开启调整keepalive的选项
	{
		GetLastSocketError("setsockopt() SO_KEEPALIVE");
		return false;
	}

	if (setsockopt(mSocket, IPPROTO_TCP, TCP_KEEPIDLE, (const char *)&keepIdle, sizeof(keepIdle)) < 0) //距离上次发送数据多长时间后开始探测
	{
		GetLastSocketError("setsockopt() TCP_KEEPIDLE");
		return false;
	}

	if (setsockopt(mSocket, IPPROTO_TCP, TCP_KEEPINTVL, (const char *)&keepInterval, sizeof(keepInterval)) < 0)//无数据交互下 每隔多长时间探测一次
	{
		GetLastSocketError("setsockopt() TCP_KEEPINTVL");
		return false;
	}
	if (setsockopt(mSocket, IPPROTO_TCP, TCP_KEEPCNT, (const char *)&keepCount, sizeof(keepCount)) < 0)//关闭一个非活跃连接之前的最大重试次数
	{
		GetLastSocketError("setsockopt() TCP_KEEPCNT");
		return false;
	}
									
	if (setsockopt(mSocket, SOL_SOCKET, SO_SNDTIMEO, (const char*)&netTimeout, sizeof(int)) < 0)//设置发送超时时间
	{
